...
from src.tasks.cue_reward import CueRewardConfig, CueRewardDataset
...
def train_epoch(
    ...
    for inputs, targets in progress:
        ...
        for t in range(inputs.shape[0]):
            ...
            pred = outputs["logits"]
            step_loss = loss_fn(pred, targets[t].unsqueeze(0))
            loss = loss + step_loss
    ...
    with context():
        for inputs, targets in dataloader:
            ...
            for t in range(inputs.shape[0]):
                outputs = model.forward_step(inputs[t], state)
                pred = outputs["logits"]
                step_loss = loss_fn(pred, targets[t].unsqueeze(0)).item()
                trial_loss += step_loss
                eta_values.append(float(outputs["eta"].item()))
                plastic_values.append(float(outputs["diagnostics"]["plastic_norm"].item()))
                if t >= targets.shape[0] // 2:
...
def execute_single_run(args: argparse.Namespace, device: torch.device, seed: int) -> Dict[str, List[Dict[str, float]]]:
    set_seed(seed)
    cue_cfg = CueRewardConfig(
        num_pairs=args.num_pairs,
        cue_dim=args.cue_dim,
        dataset_size=args.dataset_size,
    )
    train_dataset = CueRewardDataset(cue_cfg)
    val_dataset = CueRewardDataset(cue_cfg)
    train_loader = DataLoader(train_dataset, batch_size=1, shuffle=True)
    val_loader = DataLoader(val_dataset, batch_size=1, shuffle=False)

    model = PlasticTransformerModel(
        input_dim=cue_cfg.input_dim,
        output_dim=1,
    ...
    for epoch in range(train_cfg.epochs):
        train_loss, train_diag = train_epoch(model, train_loader, device, loss_fn, optimizer, train_cfg.clip_norm)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Cue-reward association experiment")
    parser.add_argument("--rule", choices=["none", "hebbian", "gradient"], default="hebbian")
    parser.add_argument("--num-pairs", type=int, default=5)
    parser.add_argument("--cue-dim", type=int, default=10)
